...
const defaultState: WindowState = { mode: 'full', alwaysOnTop: false }
...
function collectBounds(): void {
  ...
  if (b.x < -1000 || b.y < -1000 || b.width < 200 || b.height < 200) return
...
function applyMode(mode: 'mini' | 'full'): void {
  ...
  if (mode === 'mini') {
    const b = st.miniBounds
    if (b && b.x > -500 && b.y > -500 && b.width >= 350 && b.height >= 400) {
    ...
    const b = st.fullBounds
    if (b && b.x > -500 && b.y > -500 && b.width >= 800 && b.height >= 500) {
      mainWindow.setBounds(b)
    } else {
  ...
  if (isMac) {
    ...
    mainWindow.setAlwaysOnTop(st.alwaysOnTop && mode === 'mini')
  }
...
function showWindowToFront(): void {
  if (!mainWindow || mainWindow.isDestroyed()) {
    createWindow()
    return
  }
...
function createWindow(): void {
  ...
  mainWindow.webContents.on('did-fail-load', (_e, errorCode, errorDescription) => {
    debugLog(`[did-fail-load] code=${errorCode}, desc=${errorDescription}`)
  })
  mainWindow.webContents.on('render-process-gone', (_e, details) => {
    debugLog(`[render-process-gone] reason=${details.reason}, exitCode=${details.exitCode}`)
  })
  mainWindow.webContents.on('console-message', (_e, level, message, line, sourceId) => {
    debugLog(`[renderer-console] lvl=${level} msg=${message} (${sourceId}:${line})`)
  })
  ...
  mainWindow.on('close', (e) => {
    debugLog(`mainWindow close event fired, isMac=${isMac}, isQuitting=${isQuitting}`)
    if (isMac && !isQuitting) {
      e.preventDefault()
      mainWindow?.hide()
    }
...
function registerIpc(): void {
  ...
  ipcMain.on('window:setAlwaysOnTop', (_e, flag: boolean) => {
    ...
    if (isMac) {
      ...
      mainWindow?.setAlwaysOnTop(flag && st.mode === 'mini')
    }
  })
...
function registerSelectionShortcut(): boolean {
  const candidates = isWin
    ? ['Ctrl+Shift+X', 'Alt+X', 'Alt+Q', 'Ctrl+Alt+X', 'F4']
    : ['CommandOrControl+X', 'Alt+X', 'CommandOrControl+Shift+X']
...
function registerShortcuts(): void {
  ...
  let okT = false
  for (const acc of toggleCandidates) {
    try {
      const ok = globalShortcut.register(acc, () => {
        if (!mainWindow || mainWindow.isDestroyed()) return
        showMiniWindow()
  ...
  let okM = false
  for (const acc of modeCandidates) {
    try {
      const ok = globalShortcut.register(acc, () => {
        if (!mainWindow || mainWindow.isDestroyed()) return
...
app.on('before-quit', () => {
  debugLog('before-quit event fired')
  isQuitting = true
  store.flush()
})
